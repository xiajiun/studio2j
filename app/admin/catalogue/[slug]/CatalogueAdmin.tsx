...
function BrandForm({ initial, onSave, onCancel, saving }: {
  ...
  function addPost() { setForm(p => ({ ...p, posts: [...p.posts, ''] })) }
  function removePost(i: number) { setForm(p => ({ ...p, posts: p.posts.filter((_, idx) => idx !== i) })) }
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', padding: '16px', background: 'var(--beige)', borderRadius: '12px', marginBottom: '12px' }}>
      ...
      <div><label style={labelStyle}>Category</label>
        <input style={inputStyle} value={form.category} onChange={e => set('category', e.target.value)} placeholder="e.g. Illustration" /></div>
...
export function CatalogueAdmin({ brands: init, catalogueId }: { brands: CatalogueBrand[]; catalogueId: string }) {
  ...
  const visible = q
    ? init.filter(b =>
        ...
        (b.korean_name ?? '').toLowerCase().includes(q) ||
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', alignItems: 'center' }}>
        ...
        />
        <span style={{ fontFamily: 'var(--font-inter), sans-serif', fontSize: '13px', fontWeight: 300, color: 'var(--tan)' }}>{init.length} brands</span>
