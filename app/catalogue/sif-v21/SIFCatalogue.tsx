...
function BrandCard({ brand }: { brand: CatalogueBrand }) {
  ...
  const catColor = brand.category ? (CATEGORY_COLORS[brand.category] ?? '#C8A98D') : '#C8A98D'
        ...
        <div style={{ padding: '16px', flex: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
              ...
              {brand.category && (
                <span style={{ fontFamily: 'var(--font-inter), sans-serif', fontSize: '10px', fontWeight: 400, color: 'white', background: catColor, padding: '2px 8px', borderRadius: '99px' }}>
                  {brand.category}
                </span>
              )}
            </div>
          ...
          {posts.length > 0 && (
            <div style={{ position: 'relative', overflow: 'hidden', borderRadius: '10px', flex: 1 }}>
              ...
              <div style={{ display: 'flex', overflowX: 'auto', scrollSnapType: 'x mandatory', gap: '8px', paddingBottom: '4px', WebkitOverflowScrolling: 'touch' } as any}>
                {posts.map((url, i) => {
